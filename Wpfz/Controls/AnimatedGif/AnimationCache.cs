...
namespace Wpfz
{
    static class AnimationCache
    {
            ...
            private static Uri GetUri(ImageSource image)
            {
                ...
                if (image is BitmapFrame frame)
                {
                    ...
                    if (s != frame.GetType().FullName)
                    {
                        ...
                        {
                            if (fUri.IsAbsoluteUri) return fUri;
                            if (frame.BaseUri != null)  return new Uri(frame.BaseUri, fUri);
                        }
                    }
                }
                return null;
        ...
        public static void IncrementReferenceCount(ImageSource source, RepeatBehavior repeatBehavior)
        {
            ...
            _referenceCount.TryGetValue(cacheKey, out int count);
            count++;
        ...
        {
            var key = new CacheKey(source, repeatBehavior);
            _animationCache.TryGetValue(key, out ObjectAnimationUsingKeyFrames animation);
            return animation;
        }
    }
}